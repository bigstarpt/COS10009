...
class GameWindow < Gosu::Window
  ...
  def initialize
    ...
    while (column_index < x_cell_count)
      ...
      while (row_index < y_cell_count)
        ...
        print("north: #{@columns[column_index][row_index].north == nil ? 0 : 1} ")
        print("south: #{@columns[column_index][row_index].south == nil ? 0 : 1} ")
        print("east: #{@columns[column_index][row_index].east == nil ? 0 : 1} ")
        print("west: #{@columns[column_index][row_index].west == nil ? 0 : 1}")
		    puts("")
        row_index += 1
      end
      puts("-------------- End of Column --------------")
  ...
  def search(cell_x ,cell_y)
    ...
    if (cell_x == ((MAP_WIDTH / CELL_DIM) - 1))
      ...
      north_path = @columns[cell_x][cell_y - 1]
      west_path = @columns[cell_x - 1][cell_y]
      east_path = @columns[cell_x + 1][cell_y]
      south_path = @columns[cell_x][cell_y + 1]
